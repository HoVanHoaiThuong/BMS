...
namespace BMS.Class
{
    public partial class frmBook : pnSlider
    {
        ...
        private void GetBooks()
        {
            try
            {
                DataTable dt = BooksBUS.Instance.GetBookByPage(1);
        ...
        private void btSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(txtBookTitle.Text) || string.IsNullOrWhiteSpace(txtYearofPublished.Text) || string.IsNullOrWhiteSpace(txtPages.Text) || string.IsNullOrWhiteSpace(txtQuantity.Text) || string.IsNullOrWhiteSpace(txtPrice.Text))
                {
                    MetroMessageBox.Show(this, "The value cannot be empty or invalid!", "Message", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                    return;
                }
                string bookTitle = txtBookTitle.Text.Trim();
                if (txtYearofPublished.Text.Length != 4)
                {
                    MetroMessageBox.Show(this, "'Year Published' is invalid!", "Message", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;  
                }
                ...
                if (objState == EntityState.Added)
                {
                    ...
                    else
                    {
                        ...
                        btDelete.Enabled = true;
                        txtSearch.Focus();
                        MetroMessageBox.Show(this, "The image of the product may not be left blank.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
        ...
        private void txtYearofPublished_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void txtYearofPublished_TextChanged(object sender, EventArgs e)
        {
            uint x;
            if (uint.TryParse(txtYearofPublished.Text, out x) && txtYearofPublished.Text.Length > 3)
            {
                txtYearofPublished.ForeColor = Color.Black;
            }
            else
            {
                txtYearofPublished.ForeColor = Color.Red;
            }
        }
        ...
        {
            uint x;
            if (uint.TryParse(txtPages.Text, out x) && txtPages.Text.Length > 5)
            {
                txtPages.ForeColor = Color.Black;
            }
            else
            {
                txtPages.ForeColor = Color.Red;
            }
        }
        ...
        {
            uint x;
            if (uint.TryParse(txtQuantity.Text, out x))
            {
        ...
        private void txtPrice_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else if (e.KeyChar == Convert.ToChar(decimalString) && txtPrice.Text.IndexOf(decimalString) == -1)
            { }
            else
            {
                e.Handled = true;
        ...
        {
            uint x;
            if (uint.TryParse(txtPrice.Text, out x))
            {
